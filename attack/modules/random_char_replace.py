...
def attack(file_content, category='ham', replace_rate=0.1, attack_probability=0.8):
    """
    随机字符替换攻击基础方法
    ...
    if category == 'ham' and random.random() <= attack_probability:
        ...
        num_to_replace = int(len(chars) * replace_rate)
...
def execute(enron_dir, enron_attack_dir, file_callback=None):
    ...
    print(f"随机字符替换攻击完成! 总文件数: {total_count}, 攻击文件数: {attacked_count}, 攻击率: {attack_rate:.2f}%")
